...
def cache_checkout_data(request):
    """ Cache checkout data to the session. """
    try:
        # Get the pid (payment intent ID) from the POST data
    ...
    if request.method == 'POST':
        ...
        if request.user.is_authenticated:
            try:
                ...
                # Create an order form instance with the user's profile data
...
def checkout_success(request, order_number):
    ...
    if request.user.is_authenticated:
        # If the user is authenticated, get the user's profile
        profile = UserProfile.objects.get(user=request.user)
        ...
        if save_info:
            # Create a user profile form with the order data
            profile_data = {
            ...
            # Create a user profile form instance
            user_profile_form = UserProfileForm(profile_data, instance=profile)
            # Check if the form is valid
            if user_profile_form.is_valid():
                # If the form is valid, save the user profile
